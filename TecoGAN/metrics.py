...
for folder_i in range(folder_n):
    ...
    for index, i in enumerate(range(cutfr, image_no-cutfr)):
        output_img = cv2.imread(result[i])[:,:,::-1]
        target_img = cv2.imread(target[i])[:,:,::-1]

        # +++++++++++++++++++++++++++++++++++++++++++++++++++
        dif_image = (target_img - output_img)/ folder_n
        if len(diference_matrix_list) == index:
            diference_matrix_list.append(dif_image)
        else:
            diference_matrix_list[index] += dif_image

        if len(final_images) == index:
    ...
    pd.DataFrame(pd_dict).to_csv(os.path.join(FLAGS.output,"metrics_{}.csv".format(folder_i)), mode=mode)
